...
public class KillKI_new implements IPlayer {
  ...
  /**
   * berechnet neuen path zu target, wenn ein ungültiger Pfad berechnet oder das
   * ziel nicht betretbar istwird pathReset = true gesetzt nachdem ein neuer
   * pfad gesetzt wurde ist moveTarget der nächste Wegpunkt
   * @param target
   */
  private void calcPathTo(final LinkedTile target) {
    ...
    /*
     * if (myPosTile.equals(moveTarget)) //TODO muss nach update { moveTarget =
  ...
  private void explore() {
    if (pathReset)
    {
      ...
      final Vector3f targetPos = this.curPos.add(curDirection.normalize().mult(60));
      ...
      {
        final TreeMap<Integer, LinkedTile> sortedTiles = map.getUnexploredTilesSortedByDistance(curPos);
   ...
   * zu führen wenn kein gültiges Ziel existiert hält der Tank an(world.stop())
   * wenn path leer ist wird pathReset = true gesetzt
  ...
  private boolean moveToNextWaypoint() {
    ...
    // pathReset=true
    if (((moveTarget != null) && !moveTarget.isPassable) || (moveTarget == null))
    {
      pathReset = true;
      world.stop();
      return false;
      ...
      switch (wO.getType())
      {
        case Competitor:
          if (wO.getColor() != this.color)
          {
            ...
            final ShootTarget target = Battle.getShootTarget(wO.getPosition(), this.curPos);
  ...
  private void scanTerrain() {
    final List<LinkedTile> tiles = this.globalKI.getWorldMap().getTilesPossiblyInViewRange(curPos);
    for (final LinkedTile tile : tiles)
    {
      scanTile(tile);
    }
    // Direkt voraus gucken
    final Vector3f voraus = curPos.add(world.getMyDirection().normalize().mult(2));
    if (null == voraus)
    {
    ...
    if ((null != world.getTerrainNormal(voraus)) && !world.isPassable(voraus))
    {
      ...
      map.exploreTile(tileVoraus, tileVoraus.isWater(), false, tileVoraus.getNormalVector());
    }
    /*
  ...
  private void scanTile(final LinkedTile tile) {
    if (!tile.isExplored())
    {
      ...
      for (final Vector3f scanPosition : scanPositions)
      {
        ...
        {
          if (map.positionIsInViewRange(curPos, world.getMyDirection(), scanPosition))
          {
            // Ausserhalb der Map
  ...
  public String toString() {
    return "KillKI_new [color=" + color + ", name=" + name + ", world=" + world + ", blackboard=" + blackboard + ", globalKI=" + globalKI + ", perceivedObjects=" + perceivedObjects + ", objectStorage=" + objectStorage + ", map=" + map + ", spawnPos=" + spawnPos + ", curDirection=" + curDirection + ", curPos=" + curPos + ", curTile=" + curTile + ", path=" + path + ", moveTarget=" + moveTarget + ", pathReset=" + pathReset + ", flagPos=" + flagPos + ", flagPosPath=" + flagPosPath + "]";
  }
