...
public class KillKI implements IPlayer {
	...
	private final float inHangarThreshold = 15;
	...
	public void attacked(final IWorldObject competitor) {
		...
		final Vector3f direction = world.getMyPosition().clone()
				.subtract(enemy.clone()).negate();
	...
	*//**
	...
	private void calibrate() {
		final Vector3f myPosition = world.getMyPosition().clone();
		final Vector3f myDirection = world.getMyDirection().clone();
	...
	*//**
	...
	private void checkWorldObjectExistance() {
		...
		final List<LinkedTile> viewTiles = memoryMap
		...
		for (final IWorldObject obj : perceivedObjects) {
			if (mapObjects.contains(obj)) {
				final LinkedTile tile = memoryMap.getTileAtCoordinate(obj
						.getPosition());
	...
	public void defend() {
		...
		if (!blackboard.inHangar) {
			if (calcNewDefendPath) {
				final float distance = world.getMyPosition().distance(startPos);
				// float distance = 10;
				...
				for (int angle = 0; angle < 360; angle += 15) {
					...
					final Vector3f d = startPos.clone();
					d.x += x;
					d.z += z;
	...
	public void die() {
		...
		final WorldObject wO = new WorldObject(null, color, this.pos,
				EObjectTypes.Item);
	...
	public void explore() {
		...
		// if(lastPos == null){
		// direction = new Vector3f(FastMath.rand.nextInt(200),0,
		// FastMath.rand.nextInt(200));
		// } //else
		if (!blackboard.inHangar) {
			Vector3f targetPos = this.pos.add(spwanDirection.normalize().mult(
					30));
			...
			if ((null != path) && !path.isEmpty()) {
				...
				if (!targetTile.isExplored()) {
					if (targetTile.isPassable()) {
						...
						this.spwanDirection = rotateVector(this.spwanDirection,
								10);
						moveTarget = null;
					}
				} else {
					final TreeMap<Integer, LinkedTile> sortedTiles = memoryMap
							.getUnexploredTilesSortedByDistance(pos);
	...
	*//**
	...
	public float getSpeed(final float angleDeg, final float distance) {
		...
		final float time = FastMath.sqrt((distance / 49.05f)
				* FastMath.tan(angle));
	...
	*//**
	...
	public void goToHangar() {
		...
		final Vector3f myPos = world.getMyPosition().clone();
	...
	private void goToTarget(final LinkedTile target) {
		if (pathReset) {
			flagPosPath = flagPos;
			path = memoryMap.calculatePath(memoryMap.getTileAtCoordinate(pos),
					target);
			pathReset = false;
		}
		if (myPosTile.equals(moveTarget)) {
	...
	private boolean pathToFlagKnown() {
		if (memoryMap.calculatePath(memoryMap.getTileAtCoordinate(pos),
				memoryMap.getTileAtCoordinate(flagPos)).isEmpty())
			return false;
		return true;
	}
		...
		for (final IWorldObject wO : worldObjects) {
			...
			case Competitor:
				if (wO.getColor() != this.color) {
					...
					// ShootTarget target =
					// Battle.getShootTarget(wO.getPosition(), this.pos);
					// world.shoot(target.direction, target.force,
					// target.angle);
	...
	private void scanTerrain() {
		final List<LinkedTile> tiles = this.globalKI.getWorldMap()
				.getTilesPossiblyInViewRange(this.pos);
		for (final LinkedTile tile : tiles) {
			scanTile(tile);
		}
		// Direkt voraus gucken
		final Vector3f voraus = pos.add(world.getMyDirection().clone()
				.normalize().mult(2));
		if (null == voraus) {
		...
		if ((null != world.getTerrainNormal(voraus))
			...
			memoryMap.exploreTile(tileVoraus, tileVoraus.isWater(), false,
		...
		if (!tile.isExplored()) {
			...
			for (final Vector3f scanPosition : scanPositions) {
				...
				if (world.isWater(scanPosition)) {
					...
					isPassable = false;
					memoryMap
							.exploreTile(tile, isWater, isPassable, tileCenter);
					return;
				}
	...
	*//**
	...
	private boolean stuck() {
		...
		if (lastPositions.size() > 1) {
			...
			if ((comparisonPosition != null)
					&& (Math.abs(comparisonPosition.distance(pos)) < 0.01f)) {
				stuck = true;
			}
		}
	...
	public void update(final float interpolation) {
		...
		// Pruefen ob durch neue Erkundung das Zwischenziel nicht mehr betretbar
		// ist
		if (((null != moveTarget) && (!moveTarget.isPassable() || !myPosTile
				.isPassable())) || (lastTask != blackboard.curTask)) {
			world.stop();
			if (!pathReset) {
				path = null;
				moveTarget = null;
		...
		if (moveTarget != null) {
			final Vector3f newDirection = moveTarget.getTileCenterCoordinates()
					.subtract(pos);
		...
		if (stuck()) {
			System.out.println(name + " steckt fest. Sein Ziel ist "
					...
					+ memoryMap.getTileAtCoordinate(pos)
